# 04 · 更新 Windows NVIDIA 驱动

← [03 基线](03-环境现状与验收基线.md) · 下一步 → [05 重启验收](05-重启后Windows与WSL验收.md)
...
可以跳过本节与部分重启流程，直接做 Docker；vLLM 请选用 **CUDA 12.4 兼容** 镜像（见 [08](08-vLLM部署与压测.md)）。建议在笔记里写明：「决策：保持 552.12」。

---

## 前置条件

- [03](03-环境现状与验收基线.md) 已完成，Windows/WSL GPU 正常
- 当前没有重要的长时间 GPU 任务
- 能接受一次 **整机重启**
...
4. 安装结束后，安装程序常提示 **重启**。  
   → 先不要急着开一堆软件；按 [05](05-重启后Windows与WSL验收.md) 重启并验收。
...
安装完成 → **[05 - 重启后 Windows 与 WSL 验收](05-重启后Windows与WSL验收.md)**
