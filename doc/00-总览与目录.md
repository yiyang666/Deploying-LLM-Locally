...
> 面向机器：RTX 6000 Ada 48GB（或同类专业卡）· Windows + WSL2 Ubuntu 24.04  
...
|------|---------------------------|------|------|
| 通用 Agent | `Qwen/Qwen3.8-27B-FP8`（备选 `Qwen3.6-27B-FP8`） | FP8 | vLLM |
| 编程 | `Qwen/Qwen3-Coder-30B-A3B-Instruct-FP8`（备选 AWQ） | FP8 / AWQ | vLLM |

> **不要和 Qwen3.8-Max（约 2.4T MoE）搞混**：那是机房级权重，单卡 48GB 跑不动。本教程 Agent 目标是 **27B FP8**。编程侧截至文档修订时仍用 `Qwen3-Coder-30B-A3B`（尚无 3.8 Coder 开源变体）。
